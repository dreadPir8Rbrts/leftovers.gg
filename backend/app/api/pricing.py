...
def get_card_pricing(
    ...
    scrydex_row = (
        ...
        .filter(
            ...
            ScrydexPrice.fetched_at >= scrydex_cutoff,
        )
        .first()
    )
    ...
    if scrydex_row is not None:
        pj = scrydex_row.prices_json
        effective_prices = _dedup_raw_entries(pj.get("prices", []) if isinstance(pj, dict) else (pj or []))
        raw_nm_entries = [
...
def get_scrydex_prices(
    ...
    def _prices_list(pj: Any) -> List[Any]:
        return pj.get("prices", []) if isinstance(pj, dict) else (pj or [])

    cached = db.query(ScrydexPrice).filter(ScrydexPrice.card_v2_id == card_v2_id).first()
    if cached and cached.fetched_at >= stale_cutoff:
        return {"prices": _dedup_raw_entries(_prices_list(cached.prices_json))}

    if not card.external_id or card.game != "pokemon":
        return {"prices": _prices_list(cached.prices_json) if cached else []}
